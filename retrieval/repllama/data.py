...
class QueryPreProcessor:
    ...
    def __call__(self, example):
        ...
        query = self.tokenizer.encode('query: ' + example['text'],
                                      add_special_tokens=False,
                                      max_length=self.query_max_length-3,
                                      truncation=True)
...
class CorpusPreProcessor:
    ...
    def __call__(self, example):
        docid = example['docid']
        try:
            # Convert title and text to strings if they're lists
            title = example.get('title','')
            if isinstance(title, list):
                title = self.separator.join(title) if title else ""
                
            text = example.get('text','')
            if isinstance(text, list):
                text = self.separator.join(text) if text else ""
            
            # Now concatenate them
            full_text = f"passage: {title}{self.separator}{text}" if 'title' in example else f"passage: {text}"
            ...
            print(f"Error processing docid {docid}: {str(e)}")
            return {'text_id': docid, 'text': ""}
            
        return {'text_id': docid, 'text': encoded_text}
